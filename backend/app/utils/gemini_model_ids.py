...
# When rag.vision_model is unset, match legacy ImageProcessorService behavior
GEMINI_VISION_DEFAULT_LEGACY = "gemini-2.5-flash"
...
def resolve_vision_model_id(agent_config: Any) -> str:
    ...
    - google_ai_studio: ``rag.vision_model`` if set, else ``gemini-2.5-flash`` (legacy default).
    """
    ...
    if explicit:
        return explicit
    return GEMINI_VISION_DEFAULT_LEGACY
