    ...
    def _build_step_system_prompt(
        ...
        if is_mandatory:
            ...
            if getattr(step, "hard_block_until_complete", False):
                ...
                lines.append(
                    "\n⛔ ЖЁСТКОЕ ОГРАНИЧЕНИЕ — ЭТАП НЕЛЬЗЯ ПРОПУСТИТЬ:\n"
                    "Ты КАТЕГОРИЧЕСКИ не можешь отвечать ни на какие вопросы, давать советы, "
                    "информацию или обсуждать любые темы ДО завершения задачи этого этапа.\n"
                    "Если пользователь задаёт вопрос на другую тему — мягко, но твёрдо объясни, "
                    "что сначала нужно выполнить задачу этого шага. Не делай исключений.\n"
                    "Не давай никаких частичных ответов «сначала скажу, потом вернёмся». "
                    "Только выполни задачу этапа — и только после этого ты сможешь помочь."
                )
            else:
                lines.append(
